...
Backend cho hệ thống bán keycap và nhận thiết kế keycap custom theo yêu cầu khách hàng. Dự án hỗ trợ đầy đủ các luồng chính của một cửa hàng keycap: khách hàng xem sản phẩm, đặt hàng, gửi yêu cầu thiết kế riêng, trao đổi realtime với staff, duyệt mockup, thanh toán qua VNPAY sandbox và theo dõi tiến độ xử lý.
...
Dự án tích hợp VNPAY sandbox theo flow redirect:

1. Backend tạo payment URL từ order.
2. Frontend nhận `paymentUrl`.
3. Frontend redirect khách hàng sang trang thanh toán VNPAY.
4. VNPAY redirect về return URL của backend.
5. Backend verify chữ ký `vnp_SecureHash`.
6. Nếu thanh toán thành công, order được cập nhật `paymentStatus = PAID`.

Cấu hình VNPAY nằm trong `application.properties`:
...
5. Nếu chọn VNPAY, backend trả về `paymentUrl`.
6. Frontend redirect sang VNPAY để thanh toán.
...
- Không commit secret thật của SQL Server, JWT, Cloudinary, Google OAuth2 hoặc VNPAY lên repository.
