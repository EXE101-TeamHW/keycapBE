...
- Tin nhắn được lưu vào PostgreSQL.
...
Dự án sử dụng PostgreSQL. Các nhóm bảng quan trọng:
...
Hướng dẫn chạy PostgreSQL bằng Docker và deploy production nằm trong
[`DEPLOYMENT.md`](DEPLOYMENT.md).

Deploy Render nhanh bằng Blueprint: push repository lên GitHub, sau đó chọn
**New > Blueprint** trên Render và kết nối repository chứa `render.yaml`.

## Production Deployment

The backend is ready for a DigitalOcean App Platform Web Service using the
included `Dockerfile` and a Supabase PostgreSQL database. Configure secrets with
environment variables; do not edit or commit production credentials in
`application.properties`.

See [`DEPLOYMENT.md`](DEPLOYMENT.md) for the complete PowerShell local setup,
DigitalOcean settings, Supabase JDBC URL, and production environment variables.
...
- Không commit secret thật của PostgreSQL, JWT, Cloudinary, Google OAuth2 hoặc PayOS lên repository.
